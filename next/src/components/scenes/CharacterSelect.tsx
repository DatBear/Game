    ...
    <div className="flex flex-row w-fit">
      ...
      <div className="flex flex-col">
        ...
        <div className="grid grid-cols-4 gap-1 w-max">
          {weapons.map((x, idx) => <ItemSlot key={x.toString()} item={{ id: idx, stats: {}, subType: x, tier: 0 }} noDrag />)}
        </div>
        <div className="w-max">Usable Armors:</div>
        <div className="grid grid-cols-4 gap-1 w-max">
          {armors.map((x, idx) => <ItemSlot key={x.toString()} item={{ id: idx, stats: {}, subType: x, tier: 0 }} noDrag />)}
        </div>
      </div>
    </div>
